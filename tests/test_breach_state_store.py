...
def test_list_fleet_breach_history_settings(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "true")
    monkeypatch.setenv("CAS_ADMIN_API_KEY", "admin-secret")
    ...
    db = factory()
    try:
        fleet = create_fleet(db, name="Settings List Fleet")
        fleet.breach_history_retention_days = 45
        db.commit()
    finally:
        db.close()
    ...
    data = response.json()
    assert data["total"] >= 1
    row = next(item for item in data["items"] if item["fleet_name"] == "Settings List Fleet")
    assert row["retention_days"] == 45
    assert row["effective_retention_days"] == 45


def test_bulk_update_fleet_breach_history_settings(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_RETENTION_DAYS", "90")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "true")
    monkeypatch.setenv("CAS_ADMIN_API_KEY", "admin-secret")
    ...
    db = factory()
    try:
        fleet_a = create_fleet(db, name="Bulk Fleet A")
        fleet_b = create_fleet(db, name="Bulk Fleet B")
        db.commit()
        id_a, id_b = str(fleet_a.id), str(fleet_b.id)
    finally:
        db.close()
    ...
    response = ops_client.patch(
        ...
        json={
            "items": [
                {"fleet_id": id_a, "retention_days": 30},
                {"fleet_id": id_b, "retention_days": None},
            ]
        },
        headers={"X-API-Key": "admin-secret"},
    )
    ...
    row_a = next(item for item in listing["items"] if item["fleet_id"] == id_a)
    row_b = next(item for item in listing["items"] if item["fleet_id"] == id_b)
    assert row_a["retention_days"] == 30
    assert row_b["retention_days"] is None
    assert row_b["effective_retention_days"] == 90


def test_breach_history_settings_get_forbidden_for_fleet_key(ops_client, monkeypatch):
    ...
    _fleet_id, plain = _create_fleet_with_key(ops_client, monkeypatch, "Settings Forbidden Fleet")
    response = ops_client.get(
        "/api/v1/ops/fleets/breach-history-settings",
        headers={"X-API-Key": plain},
    )
...
def test_breach_history_since_until_filter(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_PUSH_ENABLED", "true")
    monkeypatch.setenv("ALERTMANAGER_URL", "http://alertmanager:9093")
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "false")
    ...
    db = factory()
    try:
        fleet = create_fleet(db, name="Date Range Fleet")
        db.commit()
        fleet_id = fleet.id
        old_row = FleetAlertBreachHistory(
            fleet_id=fleet_id,
    ...
    response = ops_client.get(
        f"/api/v1/ops/prometheus/alertmanager/breach-states/history"
        f"?fleet_id={fleet_id}"
        f"&since=2026-05-01T00:00:00Z"
        f"&until=2026-12-31T23:59:59Z"
    )
    assert response.status_code == 200
    data = response.json()
    assert data["total"] == 1
    assert data["items"][0]["alertname"] == "CASFleetHighRiskOpenAlerts"


def test_breach_history_since_after_until_422(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_PUSH_ENABLED", "true")
    monkeypatch.setenv("ALERTMANAGER_URL", "http://alertmanager:9093")
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "false")
    ...
    response = ops_client.get(
        f"/api/v1/ops/prometheus/alertmanager/breach-states/history"
        f"?fleet_id={fleet_id}"
        f"&since=2026-06-02T00:00:00Z"
        f"&until=2026-06-01T00:00:00Z"
    )
    assert response.status_code == 422


def test_breach_history_settings_csv_export(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "true")
    monkeypatch.setenv("CAS_ADMIN_API_KEY", "admin-secret")
    ...
    db = factory()
    try:
        fleet = create_fleet(db, name="CSV Settings Fleet")
        fleet.breach_history_retention_days = 60
        db.commit()
        fleet_id = str(fleet.id)
    finally:
        db.close()
    ...
    response = ops_client.get(
        "/api/v1/ops/fleets/breach-history-settings?format=csv",
        headers={"X-API-Key": "admin-secret"},
    )
    assert response.status_code == 200
    assert response.headers["content-type"].startswith("text/csv")
    body = response.text
    assert "fleet_id,fleet_name,retention_days,effective_retention_days" in body
    assert fleet_id in body
    assert "CSV Settings Fleet" in body
    assert ",60,60" in body


def test_breach_history_summary_daily_buckets(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_PUSH_ENABLED", "true")
    monkeypatch.setenv("ALERTMANAGER_URL", "http://alertmanager:9093")
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "false")
    ...
    db = factory()
    try:
        fleet = create_fleet(db, name="Summary Fleet")
        db.commit()
        fleet_id = fleet.id
        db.add(
            FleetAlertBreachHistory(
    ...
    response = ops_client.get(
        f"/api/v1/ops/prometheus/alertmanager/breach-states/history/summary?fleet_id={fleet_id}"
    )
    assert response.status_code == 200
    data = response.json()
    assert data["total_days"] == 2
    by_day = {item["day"]: item for item in data["items"]}
    assert by_day["2026-06-01"]["total"] == 2
    assert by_day["2026-06-01"]["breaching_count"] == 1
    assert by_day["2026-06-02"]["total"] == 1
    assert by_day["2026-06-02"]["breaching_count"] == 1


def test_breach_history_summary_respects_since_until(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_PUSH_ENABLED", "true")
    monkeypatch.setenv("ALERTMANAGER_URL", "http://alertmanager:9093")
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "false")
    ...
    db = factory()
    try:
        fleet = create_fleet(db, name="Summary Range Fleet")
        db.commit()
        fleet_id = fleet.id
        db.add(
            FleetAlertBreachHistory(
    ...
    assert data["items"][0]["day"] == "2026-06-01"
    assert data["items"][0]["total"] == 1


def test_breach_history_summary_admin_all_fleets(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_PUSH_ENABLED", "true")
    monkeypatch.setenv("ALERTMANAGER_URL", "http://alertmanager:9093")
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "true")
    ...
    db = factory()
    try:
        fleet_a = create_fleet(db, name="Summary Fleet A")
        fleet_b = create_fleet(db, name="Summary Fleet B")
        db.commit()
        db.add(
            FleetAlertBreachHistory(
                fleet_id=fleet_a.id,
    ...
    assert response.status_code == 200
    data = response.json()
    assert data["fleet_id"] is None
    assert data["total_days"] >= 1
    day_row = next(item for item in data["items"] if item["day"] == "2026-06-03")
    assert day_row["total"] >= 2


def test_breach_history_summary_forbidden_for_fleet_key(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_PUSH_ENABLED", "true")
    monkeypatch.setenv("ALERTMANAGER_URL", "http://alertmanager:9093")
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "true")
    ...
    fleet_id, plain = _create_fleet_with_key(ops_client, monkeypatch, "Summary Key Fleet")
    response = ops_client.get(
        "/api/v1/ops/prometheus/alertmanager/breach-states/history/summary",
        headers={"X-API-Key": plain},
    )
    ...
    response_scoped = ops_client.get(
        f"/api/v1/ops/prometheus/alertmanager/breach-states/history/summary?fleet_id={fleet_id}",
        headers={"X-API-Key": plain},
    )
...
def test_breach_history_summary_by_fleet_admin(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_PUSH_ENABLED", "true")
    monkeypatch.setenv("ALERTMANAGER_URL", "http://alertmanager:9093")
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "true")
    ...
    db = factory()
    try:
        fleet_a = create_fleet(db, name="By Fleet A")
        fleet_b = create_fleet(db, name="By Fleet B")
        db.commit()
        db.add(
            FleetAlertBreachHistory(
                fleet_id=fleet_a.id,
    ...
    response = ops_client.get(
        "/api/v1/ops/prometheus/alertmanager/breach-states/history/summary?group_by=fleet",
        headers={"X-API-Key": "admin-secret"},
    )
    assert response.status_code == 200
    data = response.json()
    ...
    names = {item["fleet_name"] for item in data["items"]}
    assert "By Fleet A" in names
    assert "By Fleet B" in names


def test_breach_history_summary_by_fleet_csv(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_PUSH_ENABLED", "true")
    monkeypatch.setenv("ALERTMANAGER_URL", "http://alertmanager:9093")
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "true")
...
def test_breach_history_summary_by_fleet_forbidden_fleet_key(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_PUSH_ENABLED", "true")
    monkeypatch.setenv("ALERTMANAGER_URL", "http://alertmanager:9093")
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "true")
    ...
    _, plain = _create_fleet_with_key(ops_client, monkeypatch, "By Fleet Key Fleet")
    response = ops_client.get(
        "/api/v1/ops/prometheus/alertmanager/breach-states/history/summary?group_by=fleet",
        headers={"X-API-Key": plain},
    )
...
def test_breach_history_summary_by_fleet_with_fleet_id_422(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_PUSH_ENABLED", "true")
    monkeypatch.setenv("ALERTMANAGER_URL", "http://alertmanager:9093")
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "true")
    ...
    response = ops_client.get(
        f"/api/v1/ops/prometheus/alertmanager/breach-states/history/summary"
        f"?fleet_id={fleet_id}&group_by=fleet",
        headers={"X-API-Key": "admin-secret"},
    )
    assert response.status_code == 422


def test_breach_history_settings_csv_import_dry_run(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "true")
    monkeypatch.setenv("CAS_ADMIN_API_KEY", "admin-secret")
    ...
    db = factory()
    try:
        fleet = create_fleet(db, name="Dry Run Fleet")
    ...
    assert data["preview"][0]["retention_days"] == 30

    db = factory()
    try:
        refreshed = db.get(Fleet, fleet_uuid)
        assert refreshed.breach_history_retention_days == before
    finally:
        db.close()


def test_breach_history_settings_csv_import_dry_run_skips_unknown(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "true")
    monkeypatch.setenv("CAS_ADMIN_API_KEY", "admin-secret")
    ...
    db = factory()
    try:
        fleet = create_fleet(db, name="Dry Run Known Fleet")
        db.commit()
        known_id = str(fleet.id)
    finally:
        db.close()
    ...
    csv_body = (
        "fleet_id,fleet_name,retention_days,effective_retention_days\n"
        f"{known_id},Dry Run Known Fleet,45,\n"
        f"{unknown_id},Missing Fleet,60,\n"
    )
    response = ops_client.post(
        "/api/v1/ops/fleets/breach-history-settings/import?dry_run=true",
...
def test_breach_history_settings_csv_import_dry_run_will_change_false(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "true")
    monkeypatch.setenv("CAS_ADMIN_API_KEY", "admin-secret")
    ...
    db = factory()
    try:
        fleet = create_fleet(db, name="Unchanged Fleet")
        fleet.breach_history_retention_days = 90
        db.commit()
        fleet_id = str(fleet.id)
    finally:
...
def test_breach_history_summary_by_fleet_name_contains(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_PUSH_ENABLED", "true")
    monkeypatch.setenv("ALERTMANAGER_URL", "http://alertmanager:9093")
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "true")
    ...
    db = factory()
    try:
        fleet_a = create_fleet(db, name="Filter Fleet Alpha")
        fleet_b = create_fleet(db, name="Filter Fleet Beta")
        db.commit()
    ...
    response = ops_client.get(
        "/api/v1/ops/prometheus/alertmanager/breach-states/history/summary"
        "?group_by=fleet&fleet_name_contains=Alpha",
        headers={"X-API-Key": "admin-secret"},
    )
    assert response.status_code == 200
    names = {item["fleet_name"] for item in response.json()["items"]}
    assert names == {"Filter Fleet Alpha"}


def test_breach_history_summary_by_fleet_name_contains_csv(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_PUSH_ENABLED", "true")
    monkeypatch.setenv("ALERTMANAGER_URL", "http://alertmanager:9093")
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "true")
    ...
    response = ops_client.get(
        "/api/v1/ops/prometheus/alertmanager/breach-states/history/summary"
        "?group_by=fleet&format=csv&fleet_name_contains=Alpha",
        headers={"X-API-Key": "admin-secret"},
    )
    assert response.status_code == 200
    assert response.headers["content-type"].startswith("text/csv")
...
def test_breach_history_settings_import_dry_run_csv(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "true")
    monkeypatch.setenv("CAS_ADMIN_API_KEY", "admin-secret")
    ...
    db = factory()
    try:
        fleet = create_fleet(db, name="Dry Run CSV Fleet")
        fleet.breach_history_retention_days = 90
        db.commit()
        fleet_id = str(fleet.id)
    finally:
    ...
    csv_body = (
        "fleet_id,fleet_name,retention_days,effective_retention_days\n"
        f"{fleet_id},Dry Run CSV Fleet,30,\n"
    )
    response = ops_client.post(
    ...
    assert response.status_code == 200
    assert response.headers["content-type"].startswith("text/csv")
    assert "will_change" in response.text
    assert "true" in response.text.lower()


def test_breach_history_settings_import_csv_without_dry_run_rejected(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "true")
    monkeypatch.setenv("CAS_ADMIN_API_KEY", "admin-secret")

    csv_body = "fleet_id,fleet_name,retention_days,effective_retention_days\n"
    response = ops_client.post(
        "/api/v1/ops/fleets/breach-history-settings/import?format=csv",
        files={"file": ("retention.csv", csv_body, "text/csv")},
        headers={"X-API-Key": "admin-secret"},
    )
    assert response.status_code == 422
...
def test_breach_history_summary_by_fleet_limit_offset(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_PUSH_ENABLED", "true")
    monkeypatch.setenv("ALERTMANAGER_URL", "http://alertmanager:9093")
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "true")
    ...
    db = factory()
    try:
        fleet_a = create_fleet(db, name="Paging Fleet A")
        fleet_b = create_fleet(db, name="Paging Fleet B")
        db.commit()
        for fleet in (fleet_a, fleet_b):
            db.add(
                FleetAlertBreachHistory(
...
def test_breach_history_summary_by_fleet_limit_offset_csv(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_PUSH_ENABLED", "true")
    monkeypatch.setenv("ALERTMANAGER_URL", "http://alertmanager:9093")
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "true")
    ...
    db = factory()
    try:
        fleet_a = create_fleet(db, name="Paging CSV Fleet A")
        fleet_b = create_fleet(db, name="Paging CSV Fleet B")
        db.commit()
        for fleet in (fleet_a, fleet_b):
            db.add(
                FleetAlertBreachHistory(
    ...
    response = ops_client.get(
        "/api/v1/ops/prometheus/alertmanager/breach-states/history/summary"
        "?group_by=fleet&format=csv&limit=1&offset=0",
        headers={"X-API-Key": "admin-secret"},
    )
    assert response.status_code == 200
    assert response.headers["content-type"].startswith("text/csv")
    ...
    assert len(lines) == 2
    assert lines[0] == "day,fleet_id,fleet_name,total,breaching_count"


def test_breach_history_settings_import_dry_run_csv_changes_only(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "true")
    monkeypatch.setenv("CAS_ADMIN_API_KEY", "admin-secret")
    ...
    db = factory()
    try:
        changed = create_fleet(db, name="Changes Only Changed")
        unchanged = create_fleet(db, name="Changes Only Unchanged")
        changed.breach_history_retention_days = 90
        unchanged.breach_history_retention_days = 90
        db.commit()
        changed_id = str(changed.id)
        unchanged_id = str(unchanged.id)
    finally:
        db.close()

    csv_body = (
        "fleet_id,fleet_name,retention_days,effective_retention_days\n"
        f"{changed_id},Changes Only Changed,30,\n"
        f"{unchanged_id},Changes Only Unchanged,90,\n"
    )
    response = ops_client.post(
        "/api/v1/ops/fleets/breach-history-settings/import"
    ...
    lines = [line for line in response.text.strip().splitlines() if line]
    assert len(lines) == 2
    assert "Changes Only Changed" in lines[1]
    assert "Changes Only Unchanged" not in response.text


def test_breach_history_settings_import_dry_run_csv_changes_only_empty(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "true")
    monkeypatch.setenv("CAS_ADMIN_API_KEY", "admin-secret")
    ...
    db = factory()
    try:
        fleet = create_fleet(db, name="Changes Only Empty")
        fleet.breach_history_retention_days = 90
        db.commit()
        fleet_id = str(fleet.id)
    finally:
    ...
    csv_body = (
        "fleet_id,fleet_name,retention_days,effective_retention_days\n"
        f"{fleet_id},Changes Only Empty,90,\n"
    )
    response = ops_client.post(
        "/api/v1/ops/fleets/breach-history-settings/import"
        "?dry_run=true&format=csv&changes_only=true",
    ...
    lines = [line for line in response.text.strip().splitlines() if line]
    assert len(lines) == 1
    assert "will_change" in lines[0]


def test_breach_history_summary_by_fleet_limit_offset_page_two(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_PUSH_ENABLED", "true")
    monkeypatch.setenv("ALERTMANAGER_URL", "http://alertmanager:9093")
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "true")
    ...
    db = factory()
    try:
        fleet_a = create_fleet(db, name="Page Two Fleet A")
        fleet_b = create_fleet(db, name="Page Two Fleet B")
        db.commit()
        for fleet in (fleet_a, fleet_b):
            db.add(
                FleetAlertBreachHistory(
...
def test_breach_history_summary_by_fleet_offset_overflow(ops_client, monkeypatch):
    monkeypatch.setenv("ALERTMANAGER_PUSH_ENABLED", "true")
    monkeypatch.setenv("ALERTMANAGER_URL", "http://alertmanager:9093")
    monkeypatch.setenv("ALERTMANAGER_BREACH_HISTORY_ENABLED", "true")
    monkeypatch.setenv("CAS_API_KEY_REQUIRED", "true")
    ...
    db = factory()
    try:
        fleet = create_fleet(db, name="Offset Overflow Fleet")
        db.commit()
        db.add(
            FleetAlertBreachHistory(
                fleet_id=fleet.id,
    ...
    response = ops_client.get(
        "/api/v1/ops/prometheus/alertmanager/breach-states/history/summary"
        "?group_by=fleet&limit=10&offset=999",
        headers={"X-API-Key": "admin-secret"},
    )
    assert response.status_code == 200
    data = response.json()
...
def test_breach_history_settings_import_dry_run_json_row_errors_mixed(ops_client, monkeypatch):
    import uuid
    ...
    db = factory()
    try:
        fleet = create_fleet(db, name="JSON Row Errors Fleet")
        fleet.breach_history_retention_days = 90
        db.commit()
        fleet_id = str(fleet.id)
    finally:
    ...
    csv_body = (
        "fleet_id,fleet_name,retention_days,effective_retention_days\n"
        f"{fleet_id},JSON Row Errors Fleet,30,\n"
        f"{unknown_id},Missing Fleet,30,\n"
    )
    response = ops_client.post(
        "/api/v1/ops/fleets/breach-history-settings/import?dry_run=true",
