"""Tests for alert-linked mitigation preview (Phase 10A)."""
...
from backend.app.db.models import AlertMitigationPreview, AuditLog, Base
...
def _seed_alert(ops_client: TestClient) -> tuple[dict, dict, str]:
    fleet = ops_client.post("/api/v1/fleets", json={"name": "Mit Fleet"}).json()
    sat = ops_client.post(
...
def test_mitigation_preview_creates_db_row(mock_find, ops_client):
    ...
    assert body["direction"] == "prograde"
    assert body["delta_v_ms"] == 0.01
    assert body["before_miss_distance_km"] >= 0
    assert body["after_miss_distance_km"] >= 0

    db = get_session_factory()()
    try:
        count = db.query(AlertMitigationPreview).count()
        assert count == 1
        audit = (
            db.query(AuditLog)
            .filter(AuditLog.action == "alert.mitigation_preview")
            .one()
        )
    ...
    listing = ops_client.get(f"/api/v1/ops/alerts?fleet_id={fleet['id']}")
    assert listing.json()["items"][0]["latest_mitigation_preview"] is not None


@patch("backend.app.services.mitigation_service.find_tle_by_norad_id")
def test_mitigation_preview_debris_not_found(mock_find, ops_client):
...
@patch("backend.app.services.mitigation_service.find_tle_by_norad_id")
def test_list_mitigation_previews(mock_find, ops_client):
    mock_find.return_value = parse_tle(DEMO_DEB)
    _fleet, _sat, alert_id = _seed_alert(ops_client)

    ops_client.post(f"/api/v1/ops/alerts/{alert_id}/mitigation-preview", json={})
    ops_client.post(f"/api/v1/ops/alerts/{alert_id}/mitigation-preview", json={})

    listing = ops_client.get(f"/api/v1/ops/alerts/{alert_id}/mitigation-previews")
...
@patch("backend.app.services.mitigation_service.find_tle_by_norad_id")
def test_get_alert_includes_latest_preview(mock_find, ops_client):
    mock_find.return_value = parse_tle(DEMO_DEB)
    _fleet, _sat, alert_id = _seed_alert(ops_client)
    ops_client.post(f"/api/v1/ops/alerts/{alert_id}/mitigation-preview", json={})
...
def test_mitigation_preview_unknown_alert(ops_client):
    response = ops_client.post(
        f"/api/v1/ops/alerts/{uuid.uuid4()}/mitigation-preview",
        json={},
    )
    assert response.status_code == 404
