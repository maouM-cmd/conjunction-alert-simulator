...
- **Phase 10AS:** purge async ポーリング UI + task status purge message
- retention dry-run プレビュー `preview CSV` エクスポート
- fleet summary 「最後へ」ボタン
...
- **Phase 10AQ:** fleet summary offset Enter キー
- `POST /prometheus/reload/history/purge` + Ops reload 履歴 purge ボタン
- dry-run JSON `row_errors[]`

## [1.49.0] - 2026-06-28

Phase 10AP — fleet offset input + reload purge beat + dry-run csv errors。

### Added

- **Phase 10AP:** fleet summary offset 数値入力 + 移動ボタン
- Celery `purge_stale_prometheus_reload_history` 定期 purge
- dry-run preview CSV `errors` 列 + エラー行
