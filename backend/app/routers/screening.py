...
def create_schedule(
    body: ScreeningScheduleCreate, db: Session = Depends(_screening_db)
) -> ScreeningScheduleOut:
...
def get_schedule(schedule_id: str, db: Session = Depends(_screening_db)) -> ScreeningScheduleOut:
...
def update_schedule(
    schedule_id: str, body: ScreeningScheduleUpdate, db: Session = Depends(_screening_db)
) -> ScreeningScheduleOut:
    sid = _parse_uuid(schedule_id, "schedule_id")
    try:
        schedule = screening_service.update_schedule(
            db, sid, **body.model_dump(exclude_unset=True)
        )
    except screening_service.ScreeningServiceError as exc:
        raise _handle_service_error(exc) from exc
    return _schedule_out(schedule)
...
def delete_schedule(schedule_id: str, db: Session = Depends(_screening_db)) -> None:
    sid = _parse_uuid(schedule_id, "schedule_id")
    try:
        screening_service.delete_schedule(db, sid)
    except screening_service.ScreeningServiceError as exc:
        raise _handle_service_error(exc) from exc
...
def trigger_schedule_run(
    schedule_id: str, db: Session = Depends(_screening_db)
) -> ScreeningRunOut:
    sid = _parse_uuid(schedule_id, "schedule_id")
    try:
        schedule = screening_service.get_schedule(db, sid)
...
def get_run(run_id: str, db: Session = Depends(_screening_db)) -> ScreeningRunOut:
