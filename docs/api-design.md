...
**遷移（Phase 10R）:** 6 状態 STM — `open` → `acknowledged` | `false_positive` | `escalated` → `acknowledged` | `mitigation_planned` | `closed` | `false_positive`

`GET /api/v1/ops/alerts/state-machine` — 6×6 遷移マトリクス。`ConjunctionAlertOut.allowed_next_statuses` で UI ボタン生成。

`ALERT_STM_AUTO_ESCALATE_STATUS=true` 時、Pc refine エスカレーションで `open` → `escalated`（10E 通知は維持）。
...
### GET /api/v1/ops/prometheus/fleet-alert-rules（Phase 10Q / 10S / 10AE / 10AF）

クエリ: `fleet_id`（optional）、`breaching_only`（optional、default false、Phase 10AE）、`breaching_fleets_only`（optional、default false、Phase 10AF）、`format`（`yaml` | `json`、default `yaml`）。
...
| `breaching_fleets_only` | 挙動（Phase 10AF） |
|-------------------------|-------------------|
| true | breaching 中の艦隊のみ rule を出力 |
...
{
  "applied": true,
  "path": "/etc/prometheus/rules/cas-fleet.yaml",
  "format": "yaml",
  "fleet_id": "uuid-or-null",
  "content": "groups:\n  ...",
...
管理者のみ。直近の Prometheus reload 履歴（`PROMETHEUS_RELOAD_HISTORY_SIZE` default 20）。

Phase 10AM 以降、`REDIS_URL` 設定時は Redis LIST `cas:prometheus:reload:history` に永続化。未設定時は in-memory。

| env | 説明 |
|-----|------|
| `PROMETHEUS_RELOAD_HISTORY_REDIS_ENABLED` | default: Redis 設定時 true |
| `PROMETHEUS_RELOAD_HISTORY_REDIS_TTL_SECONDS`（Phase 10AN） | default 604800（7日）、`0` で TTL 無効 |

Phase 10AO: TTL 超過エントリは read 時フィルタに加え、push/read 前に Redis LIST から物理 purge（`DEL` → 有効分 `LPUSH`）。in-memory も push/read 時に trim。

Phase 10AP: Celery beat `purge_stale_prometheus_reload_history`（`PROMETHEUS_RELOAD_HISTORY_PURGE_INTERVAL_SECONDS` default 86400）。
...
| `format=csv`（Phase 10AN） | dry-run 時のみ CSV 応答 |
| `changes_only`（Phase 10AO） | dry-run 時、`will_change=true` の preview 行のみ返却（CSV/JSON） |
| CSV `errors` 列（Phase 10AP） | dry-run CSV の preview 行は空、unknown fleet 等は error 行。`changes_only=true` でも error 行は残す |
| JSON `row_errors[]`（Phase 10AQ） | `{ fleet_id, message }[]`。unknown fleet のみでも 200 |

応答 `FleetBreachHistorySettingsImportOut`: `updated`, `skipped`, `errors[]`, `preview[]`（dry-run 時）

dry-run CSV 列: `fleet_id,fleet_name,retention_days,current_retention_days,effective_retention_days,will_change`

`preview[]` 各要素（Phase 10AM）: `will_change` — CSV retention と現在 override の差分有無

### PATCH /api/v1/ops/fleets/{fleet_id}/breach-history-settings（Phase 10AG）

管理者のみ。`ALERTMANAGER_BREACH_HISTORY_ENABLED=true` 前提。

```json
{ "retention_days": 45 }
```

`retention_days: null` でグローバル default（`ALERTMANAGER_BREACH_HISTORY_RETENTION_DAYS`）に戻す。範囲 1〜3650。

応答 `FleetBreachHistorySettingsOut`: `fleet_id`, `retention_days`, `effective_retention_days`
...
| クエリ（Phase 10AM） | 説明 |
|---------------------|------|
| `fleet_name_contains` | `group_by=fleet` 時、艦隊名部分一致（case-insensitive） |

| クエリ（Phase 10AN） | 説明 |
|---------------------|------|
| `limit` | `group_by=fleet` 時、1〜500（default 100） |
| `offset` | `group_by=fleet` 時、default 0 |

Ops UI（Phase 10AO）: fleet summary テーブルに前へ/次へボタン。フィルタ変更時 `offset=0` リセット。CSV ダウンロードも同一 `limit`/`offset` を付与。

Ops UI（Phase 10AP）: offset 数値入力 + 移動ボタン。`0 <= offset < total_rows` にクランプ。
...
| `fleets.breach_history_retention_days` | 艦隊別 override（Phase 10AG）。null 時はグローバル default |
| OFF（default） | 履歴 API 503、記録・purge no-op |

### DELETE /api/v1/ops/prometheus/alertmanager/breach-states/history（Phase 10AF）

retention 超過行の手動 purge。前提: push ON + history ON。

| `fleet_id` | 認可 | 挙動 |
|----------|------|------|
| 指定あり | fleet スコープ | 当該艦隊のみ purge |
| 省略 | 管理者のみ | 全 active 艦隊 purge |

応答 `FleetBreachHistoryPurgedOut`: `deleted`, `fleet_id`（optional）, `message`

Celery `purge_old_breach_history` は艦隊ループ。戻り値 `by_fleet` 付き。
